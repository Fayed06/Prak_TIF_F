...
class Queue {
    ...
    void print() {
        ...
        for (int i = 0; i < size; i++) {
            System.out.print("Pelanggan " + (i + 1) + ": " +
                    temp.arrival + ", " + temp.bursttime + "\n");
            temp = temp.next;
        }
    }
    ...
    void FCFS(int jmlhLayanan) {
        ...
        System.out.print("Jumlah layanan: " + jmlhLayanan);
        int queues = jmlhLayanan;
        ...
        while (count < size()) {
            ...
            if (wtime[qtype][0] != 0 || count == 0) {
                ...
                wtime[qtype][count] = aPrev[qtype] + bPrev[qtype] -
                        temp.arrival + waitPrev;
            ...
            queue[qtype].addLast(new Node((count + 1) + "",
                    selesai[qtype]));
            while (queue[qtype].head.selesai < temp.arrival) {
                queue[qtype].removeFirst();
            }

            System.out.printf("%8S %8S %12S %22S %10S %7S %11S %n",
                    temp.nama + " Q" + (qtype + 1), temp.arrival,
                    temp.bursttime, "Q" + (qtype + 1) + ": " +
                            queue[qtype].show(), mulai[qtype], selesai[qtype],
                    wtime[qtype][count]);
